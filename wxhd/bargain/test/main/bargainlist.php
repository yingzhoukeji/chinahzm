...
.mui-media-body{font-size:14px;}
	...
	<body>	        
        ...
        <div style="height:50px !important; clear:bothl; overflow:hidden; background:#00763D">
			...
			<ul id="list" class="mui-table-view mui-table-view-chevron" style="margin:10px 0 0 0">
				<?php
					require_once("../../include/config.db.php");
					require_once("../../include/db.class.php");
					require_once("../../include/common.php");
					$db=new db();
					$orders=$db->table("bargain")->select();
					for($i=0;$i<count($orders);$i++){
						echo '<li class="mui-table-view-cell mui-media">';
						echo '<a href="index.php?id='.$orders[$i]['id'].'&userfrom=0&param=xxx"><div class="mui-media-body">'.$orders[$i]['title'].'<p class="mui-ellipsis">开始日期：'.formatDate('Y-m-d',$orders[$i]['starttime']).'</p></div></a></li>';
					}
				?>
			</ul>
		</div>  <nav class="mui-bar mui-bar-tab">
    <a class="mui-tab-item " href="http://www.chinahzm.cn/wx/">
