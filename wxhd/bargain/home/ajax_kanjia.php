...
$db=new db();
$check=$db->table('friend')->where(array('joinid'=>$joinid,'friendopenid'=>$friendopenid))->find();
if($check){
...
if(strtotime($starttime)>strtotime(date('Y-m-d H:i:s',time()))){
	echo json_encode(array("status"=>0,"msg"=>"对不起活动还没开始"));
	exit();
}
$checkprice1=$bargain['price']-$join['price'];
if($checkprice1>$bargain['price1']){
	echo json_encode(array("status"=>0,"msg"=>"帮你砍价的人数已上限"));
	exit();
}
...
$join1=$db->table('join')->data(array("price"=>$finalprice))->where(array('id'=>(int)$joinid))->update();
Log::DEBUG("joinid1->".$join1);	
$frienddata['joinid']=$joinid;
$frienddata['friendopenid']=$friendopenid;
$frienddata['total']=$join['price'];
$frienddata['price']=$rand;
$frienddata['after']=$finalprice;
$frienddata['nickname']=$nickname;
$db->table('friend')->data($frienddata)->insert();
