...
<html>
	<head>
		...
		<style>
			...
			.mui-content{position: fixed;left: 0;right: 0;top:0;bottom: 50px;overflow-y: auto;}
...
#footer li a{ color:rgba(255,255,255,1.00);}
	...
	<script>
		...
		function getDetail(){
			var url=wxhdUrl+"/hdlist?callback=?";
			$.getJSON(url,function(json){
			
				var html='';
				for(var i=0;i<json.length;i++){
					html+='<li class="mui-table-view-cell mui-media" data-id="'+json[i].id+'">';
					html+='<a class="mui-navigate-right"><img class="mui-media-object mui-pull-left" src="'+picUrl+json[i].image+'"><div class="mui-media-body">'+json[i].title+'<p class="mui-ellipsis">分享活动 挣积分 换大森产品</p></div></a></li>';
				}
				$("#list").html(html);
				$("#list").find("li").each(function(index,el){
					$(this).click(function(){
						var id=$(this).data("id");
						window.location.href="detail.html?id="+id+"0";
					});
				});
			});
		}
